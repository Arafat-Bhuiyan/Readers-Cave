...
const BookDetails = () => {
  ...
  return (
    <div>
      <div
        key={book.bookId}
        className="flex justify-between flex-row h-full py-5 px-3 shadow-xl"
      >
        ...
        <div className="text-left w-1/2 p-5">
          ...
          <p>
            ...
            {book.tags.map((tag, index) => (
              <span
                key={index}
                className="border border-white bg-slate-100 text-green-500 font-medium text-sm px-3 py-1 ml-1 my-5 rounded-2xl"
              >
                #{tag}
              </span>
            ))}
          ...
          <div className="flex gap-2 mt-7">
            <button
              onClick={handleRead}
              className="border border-slate-400 hover:bg-slate-100 py-2 px-4 font-bold rounded-md"
            >
              Read
            </button>
            <button
              onClick={handleWishlist}
              className="border border-white bg-blue-400 hover:bg-blue-500 text-white py-2 px-4 font-bold rounded-md"
            >
              Wishlist
            </button>
          </div>
