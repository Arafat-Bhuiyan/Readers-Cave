...
const ListedBooks = () => {
  ...
  useEffect(() => {
    ...
    if (books.length > 0) {
      const readLists = books.filter((book) =>
        storedReadingList.includes(book.bookId)
      );
  ...
  useEffect(() => {
    ...
    if (books.length > 0) {
      const wishlistLists = books.filter((book) =>
  ...
  return (
    <div>
      <h2 className="text-3xl font-bold text-center py-4 bg-slate-100 rounded-lg my-4">
        Books
      </h2>
      {/* Tab navigation */}
      <div className="flex gap-5 mb-5 justify-center">
        <button
          className={`border-b-2 pb-2 ${
            activeTab === "read" ? "border-green-500" : ""
          }`}
        ...
        <button
          className={`border-b-2 pb-2 ${
            activeTab === "wishlist" ? "border-green-500" : ""
          }`}
      ...
      {activeTab === "read" ? (
        <div>
          {readLists.length > 0 ? (
            <div>
              <ul>
                {displayReadLists.map((book) => (
                  <ReadBookList key={book.bookId} book={book}></ReadBookList>
                ))}
              </ul>
            </div>
          ) : (
